...
let awkish_grammar =
  (Expr,
   function
     ...
     | Num ->
	 [[T"0"]; [T"1"]; [T"2"]; [T"3"]; [T"4"];
	  [T"5"]; [T"6"]; [T"7"]; [T"8"]; [T"9"]; [T"hello"]])

let test_1 = ((parse_prefix awkish_grammar accept_all ["4"; "+"; "5"]) = 
  Some
  ([(Expr, [N Term; N Binop; N Expr]); (Term, [N Num]); (Num, [T "4"]); 
   (Binop, [T "+"]); (Expr, [N Term]); (Term, [N Num]); (Num, [T"5"])], []))

let test_2 = ((parse_prefix awkish_grammar accept_all ["hello"]) = 
 Some ([(Expr, [N Term]); (Term, [N Num]); (Num, [T "hello"])], []) )
